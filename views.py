...
class Index(MainHandler):
    ...
    def getContext(self, **kwargs):
        ...
        user_data= get_users_number(self.get_secure_cookie("user_id"))
        if user_data:
            context.update({'users_number': user_data[0], 'user_phone':user_data[1],'user_name': user_data[2]})
        return context

    def get(self, **kwargs):
        if not self.get_secure_cookie("user_id"):
            self.set_secure_cookie("user_id",get_unique_id())
        return super(Index, self).get(**kwargs)

    def post(self, **kwargs):
        user_name = self.get_argument('user_name', '')
        phone_number = self.get_argument('phone_number', '')
        unique_id = self.get_secure_cookie("user_id")
