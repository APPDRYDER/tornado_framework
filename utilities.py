...
def setup_db():
    ...
    c.execute("GRANT SELECT on halenow.* to 'halenow_data'@'localhost';")
    c.execute("GRANT SELECT, INSERT, DELETE, UPDATE on halenow.number to 'halenow_data'@'localhost';")
    c.execute("GRANT SELECT, INSERT, DELETE, UPDATE on halenow.users to 'halenow_data'@'localhost';")
    c.execute("GRANT SELECT, INSERT, DELETE, UPDATE on halenow.status to 'halenow_data'@'localhost';")
    c.execute("flush privileges;")
    myDB.commit()
    myDB.close()
...
def send_sms(number, msg=TEXT_MESSAGE):
    if SEND_TEXTS:
        ...
        r = cursor.fetchone()
        if not r[0]:
            phone_number = number.replace(' ','').replace('(','').replace(')','').replace('-','')
            client = TwilioRestClient(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN)
            message = client.messages.create(body=msg,
                to=phone_number,    # Replace with your phone number
                from_=TWILIO_FROM_NUMB) # Replace with your Twilio number
